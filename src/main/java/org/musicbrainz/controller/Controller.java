...
public abstract class Controller extends DomainsWs2 {
	 ...
	 */
	public Controller(String applicationName, String applicationVersion, String applicationContact) {
	...
	protected boolean needsLookUp(IncludesWs2 inc) {
		...
		return (inc.isArtistRelations() || inc.isLabelRelations() || inc.isReleaseRelations()
				|| inc.isReleaseGroupRelations() || inc.isRecordingRelations() || inc.isWorkRelations()
				|| inc.isUrlRelations() || inc.isTags() || inc.isRatings() || inc.isUserTags() || inc.isUserRatings());
	}
	...
	private void updateRelations(EntityWs2 entity, EntityWs2 transit, IncludesWs2 inc) throws MBWS2Exception {
		...
		for (RelationWs2 rel : transit.getRelationList().getRelations()) {
			...
			else if (getEntity() instanceof WorkWs2 && inc.isReleaseRelations()
	...
	private WebService getDefaultQueryWs() {
		...
		return createWebService(applicationName, applicationVersion, applicationContact);
	}

	private WebService getDefaultAnnotationWs() {

		WebService adws = createWebService(applicationName, applicationVersion, applicationContact);
	...
	private WebService createWebService(String userAgentName, String userAgentVersion, String userAgentContact) {
		if (userAgentName != null) {
			return new HttpClientWebServiceWs2(userAgentName, userAgentVersion, userAgentContact);
		}
		else {
