...
public class Disc extends Controller {
	...
	private boolean needsLookUp(DiscWs2 disc, ReleaseIncludesWs2 inc) {
		...
		return (getDiscWs2() == null || super.needsLookUp(inc) || inc.isLabel() || inc.isReleaseGroups()
				|| inc.isArtistCredits() || inc.isMedia() || inc.isRecordings() || inc.isDiscids());
	}
	...
	public DiscWs2 lookUp(DiscWs2 disc) throws MBWS2Exception {
		...
		if ((disc.getToc() == null || disc.getToc().isEmpty()) && disc.getTracks() != null
				&& !disc.getTracks().isEmpty()) {
		...
		else if ((disc.getTracks() == null || disc.getTracks().isEmpty()) && disc.getToc() != null
				&& !disc.getToc().isEmpty()) {
