...
public class MediumListWs2 extends ListElement {
	...
	private List<MediumWs2> media = new ArrayList<MediumWs2>();
	...
	public List<TrackWs2> getCompleteTrackList() {
		...
		for (MediumWs2 m : getMedia()) {
			if (m.getTrackList() == null || m.getTrackList().getTracks() == null
					|| m.getTrackList().getTracks().isEmpty())
				continue;
	...
	public List<DiscWs2> getCompleteDiscList() {
		...
		for (MediumWs2 m : getMedia()) {
			if (m.getDiscList() == null || m.getDiscList().getDiscs() == null || m.getDiscList().getDiscs().isEmpty())
				continue;
