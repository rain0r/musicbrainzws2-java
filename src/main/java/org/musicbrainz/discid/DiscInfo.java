...
public class DiscInfo {
	...
	public static DiscInfo read(String device) throws DiscIdException {
		LibDiscId libdiscid = new LibDiscId(device);
		DiscInfo discId = new DiscInfo(libdiscid, device);
		libdiscid.close();
		return discId;
	}
	...
	public String toString() {
		...
		for (Integer key : trackOffsets.keySet()) {
			TrackInfo info = trackOffsets.get(key);
			msg += "  Track " + (key < 10 ? " " : "") + key + " :" + " Length: " + info.length + " - "
					+ info.getLengthString() + " (" + info.getLengthInMillis() + ")" + " Offset: " + info.offset + " - "
					+ info.getOffsetString() + " (" + info.getLengthInMillis() + ")" + "\n";
		}
		return msg;
	}

	public static DiscInfo fromOffsets(int first, int last, int[] offsets) throws DiscIdException {
		String offStr = first + " " + last;
		for (int i = 0; i < offsets.length; i++)
			offStr += " " + offsets[i];
		LibDiscId libdiscid = new LibDiscId(first, last, offsets);
		return new DiscInfo(libdiscid, "offsets: " + offStr);
