...
public class MyWebServiceImplementation extends DefaultWebServiceWs2 {
	...
	private void setConnectionParam() {
		...
		if (getUsername() != null && !getUsername().isEmpty()) {
			...
			AuthScope authScope = new AuthScope(getHost(), AuthScope.ANY_PORT, AuthScope.ANY_REALM,
	...
	protected Metadata doGet(String url) throws WebServiceException, MbXMLException {
		...
		while (repeat) {
			...
			if (md == null && trial > maxtrial) {
				String em = "ABORTED: web service returned " + HttpStatus.SC_SERVICE_UNAVAILABLE + "status '503' "
	...
	private Metadata executeMethod(HttpUriRequest method) throws MbXMLException, WebServiceException {
		...
		/*
		 * catch (WebServiceException e) { log.error("Fatal protocol violation: " +
		 * e.getMessage()); throw new WebServiceException(e.getMessage(), e); }
		 */
		catch (IOException e) {
	...
	private String buildMessage(HttpResponse response, String status) {
		...
		try {
			...
			try {
				...
				Logger.getLogger(MyWebServiceImplementation.class.getName()).log(Level.SEVERE,
						convertInputStreamToString(instream), ex);
			}
		}
